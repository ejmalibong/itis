<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "UPDATE `itinventorysystem_table` SET `actual_stock`=?, `received`=?, `for_purchase`=?, `actual_amount`=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiidi", $new_actual_stock, $new_received, $new_for_purchase, $new_actual_amount, $id);
    ...
    <div class="container">
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?id=$id"; ?>">
            <div class="form-group mb-4">
                <input type="number" class="form-control shadow" min="0" max="<?= $max_limit ?>" id="add_stock" name="add_stock" placeholder="Enter quantity to add" required>
            </div>
            ...
            <div style="display: flex; justify-content: flex-end;">
                ...
                <button type="submit" class="btn btn-success shadow" name="submit">Add Stock</button>
            </div>
        </form>
    </div>

<?php
    ...
    function add_stock_history($item, $quantity, $remarks) {
        global $conn;
        ...
        if ($stmt->execute()) {
            ...
            echo "Add shown succesfully to history.";
        } else {
            # Error adding to history and display SQL error
            echo "Error showing add to history: " . $stmt->error;
        }
    }
    ...
    if(isset($_POST['submit'])) {
        ...
        foreach($_POST['add_stock'] as $item_id => $input_num) {
            // Get the quantity to add (ensure it's a non-negative integer)
            ...
            if($input_num !== false && $input_num > 0) {
                ...
                if($row = $result->fetch_assoc()) {
                    ...
                    $new_actual_stock = $input_num + $actual_stock;
                    $new_received = $input_num + $received;
                    ...
                    $update_stmt = $conn->prepare("UPDATE itinventorysystem_table SET actual_stock = ?, received = ?, for_purchase = ?, `actual_amount`=? WHERE id = ?");
                    $update_stmt->bind_param("iiidi", $new_actual_stock, $new_received, $new_for_purchase, $new_actual_amount, $item_id);
                    $update_stmt->execute();
                    
                   // Get the input for add_stock and history_remarks
                    $item = $row['item'];
                    $history_remarks = $_POST['history_remarks'][$item_id];

                    add_stock_history($item, $input_num, $history_remarks);  
                }
            }
        }
        ...
        if($atLeastOneInputValid) {
            ...
            echo '<script>
                    alert("Need input!");
                    window.location.href = "../php/bulk_add.php";
                  </script>';
        }
    }
?>
...
<body style="background-color: #3275b9;" class="hero">
    <form method="post" action="bulk_add.php">
        ...
        <div class="container-fluid">
            <h2 class="mt-3 mb-4 text-center text-white"><strong>Add Stock</strong></h2>
            <table class="table table-hover text-center table-sm table-bordered shadow-table" id="inventoryTable">
                <thead class="table-dark table-margin">
                    <tr class="table-header">
                        ...
                        <th scope="col">Enter no. of stock to add</th>
            ...
            <?php
                while($row = $result->fetch_assoc()) {
                    ...
                    // Set the limit for the add_stock function
                    $max_stock = $row['max_stock'];
                    if($max_stock == 0) {
                        $max_limit = "";
                    }else if($max_stock > 0) {
                        $max_limit = $row['for_purchase'];
                    }
            ?>
                    <tr class="table-body table-scroll <?= htmlspecialchars($row_color) ?>">
                        ...
                        <td>
                            <input type="number" class="form-control" min="0" max="<?= $max_limit ?>" name="add_stock[<?= $row['id'] ?>]" value="" placeholder="Add Quantity">
                        </td>
                        <td>
                        <input type="text" class="form-control" name="history_remarks[<?= $row['id'] ?>]" placeholder="Remarks">
                        </td>
                    </tr>
            <?php
                }
